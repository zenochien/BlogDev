...
namespace Basics.CustomPolicyProvider
{
    ...
    public static class DynamicAuthorizationPolicyFacotry
    {
        public static AuthorizationPolicy Create(string policyName)
        {
            var parts = policyName.Split('.');
            var type = parts.First();
            var values = parts.Last();

            switch (type)
            {
                case DynamicPolicies.Rank:
                    return new AuthorizationPolicyBuilder()
                        .RequireClaim("Rank", values)
                        .Build();
                case DynamicPolicies.SecurityLevel:
                    return new AuthorizationPolicyBuilder()
                        .AddRequirements(new SecurityLevelRequirement(Convert.ToInt32(values)))
                        .Build();
                default: return null;
            }
        }
    ...
    public class SucurityLevelHandler : AuthorizationHandler<SecurityLevelRequirement>
    {
    ...
    public class CustomAuthorizationPolicyProvider : DefaultAuthorizationPolicyProvider
    {
        ...
        public override Task<AuthorizationPolicy> GetPolicyAsync(string policyName)
        {
            foreach (var customPolicy in DynamicPolicies.Get())
            {
                ...
                {
                    var policy = DynamicAuthorizationPolicyFacotry.Create(policyName);
